...
for x in range(0,count0):
    ...
    sg_default = []             # contains list of default security groups in vpc
...
###########################################################################
            ...
            if sg_response['SecurityGroups'][y]['GroupName'] == 'DOPS-LINUX-SG':             # Searches for DevOps linux instances
                gid = sg_response['SecurityGroups'][y]['GroupId']
                for tag in tags:
                        #print(tag)
                    if tag['Key'] == 'security:dops-linux' and tag['Value'] == 'true':
                        i.append(inst_id)
                        sg_inst.append(gid)
                    else:
                        continue

            if sg_response['SecurityGroups'][y]['GroupName'] == 'DOPS-WINDOWS-SG':           # Searches for windows instances
                gid = sg_response['SecurityGroups'][y]['GroupId']
                for tag in tags:
                        #print(tag)
                    if tag['Key'] == 'security:dops-windows' and tag['Value'] == 'true':
                        i.append(inst_id)
                        sg_inst.append(gid)
                    else:
                        continue
